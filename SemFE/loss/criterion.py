...
class MatchingCriterion(nn.Module):
    ...
    def fine_loss(self, preds, targets):
        ...
        """
        res = [(gt_mkpts1[torch.where((gt_mkpts0==mkp).all(1))], mkpts1[idx]) for idx, mkp in enumerate(mkpts0)]
        #gt_mkpts11 = [x.squeeze()[1:] for x, y in res if x.shape[0]!= 0]
        #mkpts11 = [y for x,y in res if x.shape[0] != 0]
        #end1 = datetime.datetime.now()
        #print(end1-start)
        gt = [(x.squeeze()[1:], y) for x, y in res if x.shape[0]!= 0]
        gt_mkpts11, mkpts11 = zip(*gt)
